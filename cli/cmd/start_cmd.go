...
var startCmd = &cobra.Command{
	...
	Run: WithResultHandler((func(_ *cobra.Command, _ []string) (string, error) {
		...
		flags := agentConfig.Flags{
			...
			ServerURL:         startParams.endpoint,
...
func init() {
	...
	startCmd.Flags().StringVarP(&startParams.endpoint, "endpoint", "e", defaultEndpoint, "set the value for the endpoint, so the CLI won't ask for this value")
